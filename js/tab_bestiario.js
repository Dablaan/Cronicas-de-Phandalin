...
window.renderBestiario = function (currentState) {
    ...
    let html = `
        ...
        <div class="grid-3" style="grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 1rem;">
    `;
    ...
    if (!bestiario || bestiario.length === 0) html += '<p class="text-muted">No hay monstruos en el tomo central.</p>';
        ...
        sorted.forEach(m => {
            html += `
                <div class="card" ondblclick="window.openQuickLook('${m.id}', 'monster')" style="cursor: pointer; position: relative; padding: 0.6rem; display: flex; flex-direction: column; ${m.isVisible ? '' : 'opacity: 0.8; border-style: dashed;'}">
                    ...
                    : `<div style="width: 100%; height: 100%; background-color: var(--leather-light); display: flex; justify-content: center; align-items: center;"><i class="fa-solid fa-dragon fa-2x text-muted"></i></div>`
                }
                    </div>
                    
                    <!-- Name + CR -->
                    <h4 style="margin: 0; font-size: 1em; font-family: 'Times New Roman', serif; font-weight: bold; color: var(--red-ink); text-transform: uppercase; line-height: 1.2;">${m.name}</h4>
                    <div style="font-size: 0.75em; font-style: italic; color: #555; margin-bottom: 0.3rem;">${m.subtitle || 'Tipo desconocido'}</div>
                    <div style="font-size: 0.85em;"><strong style="color: var(--red-ink);">CR:</strong> ${m.challenge || '-'}</div>
                    ...
                    <div style="display: flex; gap: 0.3rem; margin-top: auto; justify-content: flex-end; padding-top: 0.4rem; border-top: 1px solid var(--parchment-dark);">
                        ...
                        <button class="btn" style="padding: 0.2rem 0.4rem; font-size:0.75rem;" onclick="event.stopPropagation(); window.openEntityModal('monster', '${m.id}')" title="Editar"><i class="fa-solid fa-pen"></i></button>
                        <button class="btn btn-danger" style="padding: 0.2rem 0.4rem; font-size:0.75rem;" onclick="event.stopPropagation(); window.deleteEntity('monster', '${m.id}')" title="Eliminar"><i class="fa-solid fa-trash"></i></button>
                    </div>
                </div>
            `;
        });
